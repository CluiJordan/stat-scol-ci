  ...
  return (
    <tr style={{ background: index % 2 ? 'var(--paper-2)' : 'var(--card)' }}>
      ...
      <td style={{ ...cell, width: 85, textAlign: 'center', borderRight: 'none' }}>
        {isAdmis && <span className="mono" style={{ fontSize: 10, fontWeight: 700, color: 'var(--green-d)' }}>✓ ADMIS</span>}
        {isAbsent && <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)' }}>ABSENT</span>}
        {isAjoure && <span className="mono" style={{ fontSize: 10, color: 'var(--orange-d)' }}>AJOURNÉ</span>}
      </td>
    </tr>
  );
}
